...
- Pet Luca mini-game: interacting with Luca (facing him and pressing A) calls `lucaTalk()` which launches `petGame()` (scene/mode `pet`). A zoomed-in Luca (scale 5.5, box `PET_LX/PET_LY`) fills the screen and you stroke him with the cursor or a finger (pointermove/pointerdown listeners map client coords onto the 240x160 canvas; the system cursor is hidden and replaced by a little hand). Stroking fills a heart love meter; his sparkling favourite spot (`PET_SPOTS`: ears, head, belly, chin) fills it faster and rotates. A press is a keyboard fallback pat. At 100% Luca flops for a cuddle finale with a heart shower. First cuddle per map grants happy (tracked by `G.lucaPat[mapId]`); replays give a tiny bump. Works anywhere Luca appears (bedroom, and the weekend maps where he tags along).
